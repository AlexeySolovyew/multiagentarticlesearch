...
<form action="check_solution.php" method="POST">
    <input type="hidden" name="solutionid" value="<?echo $_POST['solutionid'];?>">
    Номер задачи: <?echo $row[HometaskID]." - ".$row[TaskForHometask]?><br/>
    Условие: <?echo $row[Condition]?><br/>
    Текст решения: <br>
    <textarea id="code" rows="15" cols="140" name="code"><?echo $row['Code'];?></textarea><br>
...
<form action="add_test.php" method="post">
    <input type="hidden" name="taskid" value="<?echo $row[TaskID]?>">
